...
pub fn get_func(filename: &str, func: xlua::FuncName) -> mlua::Result<Option<mlua::Function>> {
    INSTANCE.with(|t| {
        let key = normalize_file_name(filename);
        t.load(&key)?;
        Ok(t.map.borrow().get(&key).unwrap()[func as usize].clone()) // safe: key is loaded just now
    })
}

pub fn create_table() -> mlua::Result<mlua::Table> {
    INSTANCE.with(|t| t.lua.create_table())
}
...
impl LuaManager {
    ...
    fn load(&self, filename: &str) -> mlua::Result<()> {
        // 检查是否已加载
        let key = normalize_file_name(filename);
